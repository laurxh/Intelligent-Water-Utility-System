...
class NNModel(ModelStrategy):
    ...
    def get_recent_data(self, dataset):
        ...
        print(f"T_x value: {self.T_x}")
        dataset.loc[:, dataset.columns != 'Date'] = self.standard_scaler.transform(dataset.loc[:, dataset.columns != 'Date'])
        # 直接设置预测起始日期
        self.forecast_start = dataset['Date'].iloc[-1]
        print(f"预测起始日期: {self.forecast_start}")
        # 直接创建最后一个窗口作为输入
        features = dataset.loc[:, dataset.columns != 'Date'].values
        last_window = features[-self.T_x:].reshape(self.T_x, -1)
    
        print(f"手动创建的输入窗口形状: {last_window.shape}")
